...
function getItemLabelFromType(tastingType?: string): string {
  return getItemLabel({ tasting_type: tastingType });
}

function getItemName(
  beerNumber: number,
  revealByNumber: Map<number, BeerReveal>,
  itemLabel: string
): string {
  return revealByNumber.get(beerNumber)?.beer_name ?? `${itemLabel} #${beerNumber}`;
}
...
export function ScorecardsContent({
  ...
  const itemLabel = getItemLabelFromType(tastingType);
  const revealByNumber = useMemo(
    () => new Map(reveals.map((r) => [r.beer_number, r])),
    [reveals]
  );
  ...
  const playerScorecards = useMemo(
    () =>
      players.map((player) => {
        ...
        const withScores = playerRatings.filter(
          (r) => r.taste != null && r.crushability != null
        );

        const overallRanked = [...withScores]
          .map((r) => ({
            beerNumber: r.beer_number,
            name: getItemName(r.beer_number, revealByNumber, itemLabel),
        ...
        const guessAccuracy = playerRatings
          ...
          .map((r) => {
            const beerName =
              revealByNumber.get(r.beer_number)?.beer_name ??
            ...
            let result: "✅" | "❌" | "—" = "—";
            if (guess) {
              result = actual && guess.toLowerCase() === actual ? "✅" : "❌";
            }
        ...
        const avgTaste =
          withScores.length > 0
            ? withScores.reduce((s, r) => s + (r.taste ?? 0), 0) /
              withScores.length
            : 0;
        const avgCrush =
          withScores.length > 0
            ? withScores.reduce((s, r) => s + (r.crushability ?? 0), 0) /
              withScores.length
            : 0;

        const withGuess = playerRatings.filter(
          (r) => r.guess != null && r.guess.trim() !== ""
        );
    ...
    [players, ratings, revealByNumber, itemLabel]
  );
          ...
          >
            ...
            <div className="p-4 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <h3 className="text-sm font-semibold text-[var(--amber-gold)] mb-2">
                    Overall
                  </h3>
                  <ol className="text-sm text-[var(--text-body)] space-y-1 list-decimal list-inside">
                    ...
                    {overallRanked.length === 0 && (
                      <li className="list-none text-[var(--text-muted)]">
                        No ratings
                      </li>
                    )}
                  </ol>
              ...
              <div className="pt-2 border-t border-[var(--border-amber)] text-sm text-[var(--text-muted)]">
                ...
                <span className="text-[var(--text-heading)] font-medium">
                  {avgTaste.toFixed(1)}/10
                </span>
                {" · "}
                Avg crushability:{" "}
                <span className="text-[var(--text-heading)] font-medium">
                  {avgCrush.toFixed(1)}/10
                </span>
                {" · "}
                Correct guesses:{" "}
                <span className="text-[var(--text-heading)] font-medium">
                  {correctGuesses} out of {totalGuesses}
                </span>
              </div>
            </div>
          </section>
